...
char BMPRGBA_BMP_Palette[256][4] = 
{
    {0x00, 0x00, 0x00, 0xff}, {0x00, 0x00, 0x00, 0x00}, {0x00, 0x80, 0x00, 0xff}, {0x00, 0x80, 0x80, 0xff}, 
...
//--------------------------------------------------------------------
// Get information of BMP image
//--------------------------------------------------------------------
void BMPRGBA_BmpGetInf(BMPRGBA_BmpInf *bmpinf,FILE *bmp_file)
{
    char bmp_head_data[54] = ""; //Data of BMP file head
...
void BMPRGBA_BmpToRGBA(char *dest,BMPRGBA_BmpInf *bmpinf,FILE *bmp_file)
{
    ...
    if(bmp_b == 8)
    {
        ...
        fseek(bmp_file,1078,SEEK_SET);
        fread(bmp_data_cache,bmp_size,1,bmp_file);
        while(bmp_pxi < bmp_size)
        {
            int pal_i = bmp_data_cache[bmp_pxi] & 0x000000ff;
            dest[dest_i*4] = BMPRGBA_BMP_Palette[pal_i][2];
            dest[dest_i*4+1] = BMPRGBA_BMP_Palette[pal_i][1];
            dest[dest_i*4+2] = BMPRGBA_BMP_Palette[pal_i][0];
            dest[dest_i*4+3] = BMPRGBA_BMP_Palette[pal_i][3];
            bmp_pxx ++;
            bmp_pxi ++;
            dest_i ++;
            if(bmp_pxx == bmp_w)
    ...
    if(bmp_b == 32)
    {
        ...
        while(bmp_pxi < bmp_size)
        {
            dest[dest_i*4] = bmp_data_cache[bmp_pxi+1];
            dest[dest_i*4+1] = bmp_data_cache[bmp_pxi+2];
            dest[dest_i*4+2] = bmp_data_cache[bmp_pxi+3];
            dest[dest_i*4+3] = bmp_data_cache[bmp_pxi];
            bmp_pxi += 4;
            dest_i ++;
        }
    }
